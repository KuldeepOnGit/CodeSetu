...
function updateAuthButton(isLoggedIn) {
    ...
    if (isLoggedIn) {
        ...
        loginButton.onclick = logoutFarmer;
    } else {
...
async function loadLoggedInFarmer() {
    const accessToken =
    ...
    try {
        ...
        if (!farmer) {
            throw new Error(
                "Authenticated farmer data was not received."
            );
        }
...
async function loadCenters() {
    try {
        ...
        if (!response.ok) {
            throw new Error("Centers API failed");
        }

        const centers = await response.json();

        console.log("Live Centers:", centers);

        const centerGrid = document.getElementById("centerGrid");

        if (!centerGrid) {
            console.error("centerGrid not found");
            return;
        }
        ...
        centers.forEach(center => {
            ...
            const capacityPercent = center.daily_capacity
                ? Math.round(
                    (center.current_queue / center.daily_capacity) * 100
                )
                : 0;

            let statusText = center.status || "Unknown";
            ...
            card.innerHTML = `
                ...
                <div class="center-details">
                    ...
                    <div>
                        <small>Queue</small>
                        <strong>${center.current_queue || 0}</strong>
                    </div>

                    <div>
                        <small>Waiting</small>
                        <strong>${(center.current_queue || 0) * 10} Min</strong>
                    </div>
                ...
                <button
                    class="outline-btn"
                    onclick="selectCenter('${center.center_id}')"
                >
                    Select Center
                </button>
            `;
